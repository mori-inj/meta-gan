...
def resize(image, w, h):
    ...
    for i in range(h):
        for j in range(w):
            img[i,j] = np.average(image[int(i*H/h):int((i+1)*H/h), int(j*W/w):int((j+1)*W/w)])
    return img
...
for i in range(train_x.shape[0]):
    ...
    img = resize(img, 28//shrink_size, 28//shrink_size)
    resized_train_x.append( np.reshape(img, (28//shrink_size)*(28//shrink_size)) )
train_x = np.asarray(resized_train_x)

image_size = (28//shrink_size)*(28//shrink_size)
        ...
        with tf.Session(graph = g) as sess:
            ...
            for epoch in range(total_epochs):
                ...
                if epoch % 5 == 0:
                    ...
                    sample_num = 10
                    sample_noise = random_noise(sample_num)
                    ...
                    for i in range(sample_num):
                        for j in generated[i]:
                            ...
                            f.write("%0.2f " % j)
                            cnt+=1
                            if cnt % (28//shrink_size) == 0:
                                f.write('\n') # sample_out += "\n"
                        f.write('\n') # sample_out += "\n"
                    f.close() # print(sample_out) 
                """
            
            np.set_printoptions(threshold=np.nan)
            w = sess.run(g_vars)
            #f = open('./weight_half/'+str(target_class)+"_"+sys.argv[2]+'.txt','w')
            f = open('./weight_half/'+str(target_class)+"_"+str(iteration)+'.txt','w')
